...
class CodingPrompts:
    ...
    def format_system_msg():
        return textwrap.dedent(
            ...
            READ THE TOOL RESPONSE. A successful edit returns exactly "Change applied successfully." Any response that starts with "Error:" or contains "No changes were made" means **the change was not applied**. The most common cause is that your `old_str` doesn't match the file byte-for-byte — usually a whitespace or indentation difference. When that happens:
              1. Call `view_file` on the target path to see the exact lines (with leading whitespace) around where you want to edit.
              2. Copy the exact text from the view_file output as your new `old_str`.
              3. Retry `str_replace` with the corrected `old_str`.
            Do not declare success while the most recent edit-tool response was an error. Do not re-issue the same `old_str` after it failed — change the `old_str` first, otherwise the agent will dedup the identical call and you'll burn an iteration for nothing.
